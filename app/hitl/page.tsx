  ...
  return (
    <>
      <PageHeader 
        ...
        actions={
          <div className="flex items-center gap-2 text-xs text-ink-300">
            <div className="w-6 h-6 rounded-full bg-accent-700/30 border border-accent-700/50 flex items-center justify-center text-[10px] font-semibold text-accent-300">김</div>
            <span>김정민 ESG팀장</span>
          </div>
        }
      />
      ...
      <div className="flex h-[calc(100vh-105px)]">
        {/* 왼쪽: 검토 대기 큐 */}
        <aside className="w-80 shrink-0 border-r border-ink-700 bg-ink-900/50 overflow-y-auto">
          <div className="p-4 border-b border-ink-700 sticky top-0 bg-ink-900/90 backdrop-blur">
            <div className="text-[10px] uppercase tracking-wider text-ink-400 mb-1">검토 대기열</div>
            <div className="text-xl font-semibold num-mono text-ink-50">{reviewQueue.length}<span className="text-xs text-ink-400 ml-1">건</span></div>
          </div>
          ...
          <div className="divide-y divide-ink-700/60">
            {reviewQueue.map(c => (
              <button
                ...
                className={clsx(
                  'w-full text-left p-4 hover:bg-ink-800/50 transition-colors',
                  selectedCase.id === c.id && 'bg-accent-700/10 border-l-2 border-accent-500'
                )}
              >
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-xs font-semibold num-mono text-ink-100">{c.batchId}</span>
                  <PriorityDot priority={c.priority} />
                </div>
                <div className="text-[11px] text-ink-300 mb-2 truncate">{c.supplier}</div>
                <div className="flex items-center gap-1.5 mb-2">
                  <Badge tone="neutral" size="sm">→ {c.destination}</Badge>
                  <VerdictBadge verdict={c.agentVerdict} />
                </div>
                <div className="text-[10px] text-ink-500 num-mono flex items-center justify-between">
                  <span>신뢰도 {c.confidence}</span>
        ...
        <main className="flex-1 overflow-y-auto">
          <div className="p-8 max-w-5xl mx-auto space-y-5">
            ...
            {/* 케이스 헤더 */}
            <div className="rounded-sm border border-ink-700 bg-ink-800/40 p-5">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <div className="flex items-center gap-2 mb-1.5">
                    <span className="text-lg font-semibold num-mono text-ink-50">{selectedCase.batchId}</span>
                    <Badge tone="neutral">→ {selectedCase.destination}</Badge>
                    <VerdictBadge verdict={selectedCase.agentVerdict} />
                  </div>
                  <p className="text-sm text-ink-300">{selectedCase.supplier}</p>
                </div>
                <div className="text-right text-[11px] text-ink-400 num-mono">
                  <div>접수 {selectedCase.receivedAt}</div>
            ...
            <Card 
              ...
              action={
                <button onClick={() => setReasonExpanded(!reasonExpanded)} className="text-ink-400 hover:text-ink-200">
            ...
            >
              {reasonExpanded && (
                <div className="space-y-3">
                  <div className="rounded-xs border border-amber-700/30 bg-amber-500/5 p-3 flex items-start gap-2.5">
                    <AlertTriangle className="w-4 h-4 text-amber-400 shrink-0 mt-0.5" />
                    <div className="text-sm text-ink-200 leading-relaxed">
                      <span className="font-semibold text-amber-300">{selectedCase.triggerReason}</span>
                  ...
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    <div className="rounded-xs bg-ink-900/60 p-3">
                      <div className="text-[10px] uppercase tracking-wider text-ink-400 mb-1.5">에이전트 잠정 판정</div>
                      <div className="text-ink-200">위반 가능성 있음 (확신 부족)</div>
                    </div>
                    <div className="rounded-xs bg-ink-900/60 p-3">
                      <div className="text-[10px] uppercase tracking-wider text-ink-400 mb-1.5">권장 조치</div>
                      <div className="text-ink-200">협력사에 추가 증빙 요청 검토</div>
                    </div>
                  </div>
                </div>
              )}
            ...
            <Card 
              ...
              action={
                <button onClick={() => setEvidenceExpanded(!evidenceExpanded)} className="text-ink-400 hover:text-ink-200">
            ...
            <Card title="검토 결정" subtitle="판단을 내리고 사유를 기록해 주세요">
                ...
                <button 
                  ...
                  className={clsx(
                    ...
                    decision === 'approve' 
                      ...
                      : 'border-ink-700 bg-ink-900/40 hover:border-emerald-700/50'
                  )}
                >
                  <div className="flex items-center gap-2 mb-1.5">
                    <CheckCircle2 className={clsx('w-5 h-5', decision === 'approve' ? 'text-emerald-400' : 'text-ink-500')} />
                    <span className={clsx('text-sm font-semibold', decision === 'approve' ? 'text-emerald-300' : 'text-ink-200')}>
                      승인
                    </span>
                  </div>
                ...
                <button 
                  ...
                  className={clsx(
                    ...
                    decision === 'reject' 
                      ...
                      : 'border-ink-700 bg-ink-900/40 hover:border-red-700/50'
                  )}
                >
                  <div className="flex items-center gap-2 mb-1.5">
                    <XCircle className={clsx('w-5 h-5', decision === 'reject' ? 'text-red-400' : 'text-ink-500')} />
                    <span className={clsx('text-sm font-semibold', decision === 'reject' ? 'text-red-300' : 'text-ink-200')}>
                      반려
                    </span>
                  </div>
                  <p className="text-[11px] text-ink-400">
              ...
              <div className="space-y-2">
                <label className="text-[10px] uppercase tracking-wider text-ink-400">결정 사유 (감사 기록에 영구 저장)</label>
                <textarea 
                  rows={3}
                  placeholder="이 결정의 근거를 명확히 기록해 주세요. 향후 규제 당국 감사 시 활용됩니다."
                  className="w-full px-3 py-2.5 rounded-xs bg-ink-900 border border-ink-700 text-sm text-ink-100 focus:border-accent-500 outline-none resize-none"
                />
              </div>

              {/* 추가 조치 옵션 */}
              <div className="mt-4 pt-4 border-t border-ink-700">
                <div className="text-[10px] uppercase tracking-wider text-ink-400 mb-2">추가 조치</div>
                <div className="space-y-2">
              ...
              <div className="mt-5 flex items-center gap-2">
                ...
                <button className="px-4 py-2.5 rounded-xs border border-ink-600 text-xs text-ink-300 hover:bg-ink-700 transition-colors">
                  나중에 결정
                </button>
              </div>
            </Card>
...
function MetricBox({ label, value, tone }: any) {
  ...
  return (
    <div>
      ...
      <div className={clsx('text-sm font-semibold num-mono', colors[tone] || 'text-ink-100')}>
        {value}
      </div>
    </div>
  );
...
function EvidenceRow({ icon: Icon, nodeType, nodeName, finding, detail, severity }: any) {
  const sevColors: any = {
    ok:    'border-emerald-700/30 bg-emerald-500/5',
    warn:  'border-amber-700/30 bg-amber-500/5',
    alert: 'border-red-700/30 bg-red-500/5',
    info:  'border-ink-700/60 bg-ink-900/40',
  };
  const iconColors: any = {
    agent: 'text-accent-400 bg-accent-700/20',
    tool: 'text-blue-400 bg-blue-700/20',
  };
    ...
    <div className={clsx('rounded-xs border p-3', sevColors[severity])}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-xs text-ink-200">{finding}</div>
          <div className="text-[11px] text-ink-400 mt-1 num-mono">{detail}</div>
        </div>
      </div>
    </div>
  );
...
function FileLink({ name, type, pages }: any) {
  return (
    <button className="flex items-center gap-2.5 p-2.5 rounded-xs border border-ink-700/60 hover:border-accent-700/40 hover:bg-ink-900/60 transition-colors text-left">
      <FileText className="w-4 h-4 text-ink-400 shrink-0" strokeWidth={1.5} />
      <div className="flex-1 min-w-0">
        <div className="text-[11px] font-medium text-ink-100 truncate">{name}</div>
        <div className="text-[10px] text-ink-500">{type} · {pages}p</div>
      </div>
...
function CheckOption({ label, defaultChecked }: any) {
  return (
    <label className="flex items-center gap-2.5 cursor-pointer group">
      ...
      />
      <span className="text-xs text-ink-300 group-hover:text-ink-100">{label}</span>
    </label>
  );
}
